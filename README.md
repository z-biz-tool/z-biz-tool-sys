...
- 🖥️ **CPU** — 总使用率 + 每核心使用率
- 🧠 **内存** — 已用 / 总量 / 压力状态
- 💾 **磁盘** — 各分区容量与使用率 + 读写速率（IOKit 块设备计数按刷新窗口取增量，已与 `iostat` 逐帧对账；拿不到计数的平台显示 `—`）
...
- 🔕 触发时应用内 Toast（critical 红色 / warning 黄色）+ 抽屉内的**落盘告警历史**列表；后端阈值会夹取脏输入并把真正生效的值回给前端覆盖，界面不会显示一份"后端其实没接受"的配置
...
- 🧪 采集不到就明说"没有数据"，不颁一个 0 % 的冠军、不编一个数；温度/风扇这类未实现的能力会直接回答"本机还没有温度/风扇采集通路"
...
| 前端包体 | 单 chunk 1,579.71 kB / gzip 489.38 kB（实测 `dist/assets/index-zu9E1Kap.js` 1 579 711 B；上一轮基线 1 570 157 B ⇒ 迷你模式 + 阈值抽屉去废弃净 +9,554 B / +2.69 kB gzip），尚未做代码分割（不在规划任务里） |
| DNS 刷新 | 只执行用户态命令；平台要求提权时返回 `manualCommand` 由用户自行执行，应用内零 `sudo` |
| 告警 | 引擎、阈值配置与**落盘历史**已落地（判定在后端采集循环，见上"资源告警"）。**未做**：系统通知（需引入 `tauri-plugin-notification` 并扩 `capabilities`，当前是 8 条显式权限、无通配项，先确认再动）、演示/全屏自动静默（只有手动总开关）、磁盘趋势图上的告警标注（磁盘告警只写在磁盘相关界面，概览两张图没有磁盘曲线） |
| 温度 / 风扇 | 未实现（方案 T5-08）：macOS 侧要走 IOKit/SMC 私有通路，代价与风险都还没核。诊断助手对温度类问题**如实回答"本机还没有温度/风扇采集通路"**，不给一个编出来的数 |
