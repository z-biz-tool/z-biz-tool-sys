...
| 前端包体 | 单 chunk 1,553.48 kB / gzip 481.12 kB（实测 `dist/assets/index-BJ7uSxo-.js` 1 553 479 B），尚未做代码分割（不在规划任务里） |
| DNS 刷新 | 只执行用户态命令；平台要求提权时返回 `manualCommand` 由用户自行执行，应用内零 `sudo` |
| 告警 | 引擎、阈值配置与**落盘历史**已落地（判定在后端采集循环，见上"资源告警"）。**未做**：系统通知（需引入 `tauri-plugin-notification` 并扩 `capabilities`，当前仅 `core:*` + `dialog:*`，先确认再动）、演示/全屏自动静默（只有手动总开关）、磁盘趋势图上的告警标注（磁盘告警只写在磁盘相关界面，概览两张图没有磁盘曲线） |
| 温度 / 风扇 / 菜单栏迷你模式 / 自然语言 Agent | 未开始（Phase 5 规划中） |
